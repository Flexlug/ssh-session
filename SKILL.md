...
Драйвер: `~/.claude/skills/ssh-session/scripts/sshsess.py` — исполняемый, можно
запускать напрямую (Python 3, только stdlib: ни tmux, ни pip-пакетов). В примерах
ниже он сокращён до `$S`; задавай `S=...` в начале того же вызова Bash — между
отдельными вызовами переменные не сохраняются, там подставляй полный путь. Если
локальная оболочка fish, присваивание другое: `set S ~/.claude/...`.
...
```bash
S=~/.claude/skills/ssh-session/scripts/sshsess.py   # в fish: set S ~/.claude/...
